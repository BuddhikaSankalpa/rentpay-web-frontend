...
export default function Support() {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            case 'closed':
                return { color: 'text-gray-400', bg: 'bg-white/5', border: 'border-white/10', icon: <MdCheckCircle />, text: 'Closed' };
            case 'done':
                return { color: 'text-[#A58ED4]', bg: 'bg-[#A58ED4]/10', border: 'border-[#A58ED4]/20', icon: <MdDoneAll />, text: 'Awaiting Admin' };
            case 'in_progress':
            ...
            default:
                return { color: 'text-orange-400', bg: 'bg-orange-500/5', border: 'border-orange-500/20', icon: <MdPendingActions />, text: 'Pending' };
        }
    };
    ...
    return (
        <div className="w-full mt-8 flex flex-col gap-8 pb-10">
            ...
            <div className="bg-[#0a0815]/60 backdrop-blur-md border border-white/[0.06] rounded-2xl p-8 flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-serif text-white tracking-wide flex items-center gap-3">
                        <MdSupportAgent className="text-[#A58ED4] text-4xl" /> Support & Maintenance
                    </h2>
                    <p className="text-gray-400 mt-2 text-sm font-serif italic ml-12">Submit your woes. The administration will attend to them... eventually.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <div className="bg-[#0a0815]/60 backdrop-blur-md border border-white/[0.06] rounded-2xl p-6 sticky top-6">
                        <h3 className="text-[11px] font-bold tracking-[0.2em] text-gray-500 uppercase mb-8 flex items-center gap-2">
                            <MdBuild className="text-gray-400 text-lg" /> New Request
                        </h3>

                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div>
                                <label className="block text-[10px] font-bold tracking-widest text-gray-500 uppercase mb-2">Issue Category</label>
                                <div className="relative">
                                    ...
                                    <select
                                        ...
                                        className="w-full pl-12 pr-4 py-3.5 bg-white/[0.02] border border-white/[0.06] text-gray-300 rounded-xl focus:border-[#A58ED4] outline-none transition-all appearance-none"
                                    >
                                        <option value="Electrical" className="bg-[#0a0815]">Electrical</option>
                                        <option value="Plumbing" className="bg-[#0a0815]">Plumbing</option>
                                        <option value="Internet / Wi-Fi" className="bg-[#0a0815]">Internet / Wi-Fi</option>
                                        <option value="Furniture" className="bg-[#0a0815]">Furniture</option>
                                        <option value="Cleaning" className="bg-[#0a0815]">Cleaning</option>
                                        <option value="Other" className="bg-[#0a0815]">Other</option>
                                    </select>
                                </div>
                            </div>

                            <div>
                                <label className="block text-[10px] font-bold tracking-widest text-gray-500 uppercase mb-2">Subject</label>
                                <input
                                    type="text"
                                    value={formData.subject}
                                    onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
                                    placeholder="e.g. Flickering lights"
                                    className="w-full px-4 py-3.5 bg-white/[0.02] border border-white/[0.06] text-gray-200 rounded-xl focus:border-[#A58ED4] outline-none transition-all placeholder-gray-600"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows="5"
                                    placeholder="Explain the nightmare in detail..."
                                    className="w-full px-4 py-3.5 bg-white/[0.02] border border-white/[0.06] text-gray-200 rounded-xl focus:border-[#A58ED4] outline-none transition-all resize-none placeholder-gray-600"
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full mt-2 h-12 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl flex items-center justify-center gap-2 transition-all text-[10px] font-bold tracking-[0.2em] uppercase text-white disabled:opacity-50"
                            >
                                {isSubmitting ? 'Summoning Help...' : 'Submit Request'}
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2">
                    <div className="bg-[#0a0815]/60 backdrop-blur-md border border-white/[0.06] rounded-2xl p-6 min-h-[500px]">
                        <h3 className="text-[11px] font-bold tracking-[0.2em] text-gray-500 uppercase mb-8 flex items-center gap-2">
                            <MdHistory className="text-gray-400 text-lg" /> My Request History
                        </h3>

                        {loading ? (
                            <div className="text-center py-10 text-gray-500 font-serif italic">Checking archives...</div>
                        ) : tickets.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-64 text-gray-600 border-2 border-dashed border-white/5 rounded-2xl">
                                ...
                                <p className="font-serif italic text-sm">No maintenance requests made... yet.</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {tickets.map((ticket) => {
                                    ...
                                    return (
                                        <div key={ticket.id} className="bg-white/[0.02] border border-white/[0.04] hover:border-white/10 rounded-xl p-6 transition-all">

                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 rounded-full bg-black/40 flex items-center justify-center text-gray-500 text-2xl border border-white/5 shrink-0">
                                                        {getCategoryIcon(ticket.category)}
                                                    </div>
                                                    <div>
                                                        <h4 className="text-white font-serif text-xl tracking-wide">{ticket.subject}</h4>
                                                ...
                                                <div className={`px-3 py-1.5 rounded-full border flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest whitespace-nowrap ${statusObj.bg} ${statusObj.border} ${statusObj.color}`}>
                                                    {statusObj.icon} {statusObj.text}
                                                </div>
                                            </div>

                                            <div className="text-sm text-gray-400 leading-relaxed border-t border-white/[0.04] pt-4 mt-4">
                                                {ticket.description}
                                            </div>

                                            {ticket.status === 'in_progress' && (
                                                <div className="mt-5 pt-5 border-t border-white/[0.04]">
                                                    <p className="text-xs text-gray-500 mb-3 font-serif italic">
                                                        Has the issue been resolved? Confirm below to notify the administration.
                                                    </p>
                                                    <button
                                                        onClick={() => handleMarkDone(ticket.id)}
                                                        className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10 px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-colors"
                                                    >
                                                        <MdThumbUp className="text-[#A58ED4]" /> Yes, it's fixed
                                                    </button>
                                                </div>
                                            )}

                                            {ticket.status === 'done' && (
                                                <div className="mt-5 pt-5 border-t border-white/[0.04] text-[11px] tracking-widest uppercase font-bold text-[#A58ED4]/80">
                                                    ✓ You confirmed the fix. Waiting for admin closure.
                                                </div>
                                            )}
                                        </div>
                                    );
